# Basic Consume user and password based-authentication example
...
service, `channelUrl`, and credentials that the client uses to authenticate
itself to the service, `channelUsername` and `channelPassword`. It also includes a certificate, `verifyCertificateBundle` , and HTTP proxy settings `httpProxySettings` created previously.
